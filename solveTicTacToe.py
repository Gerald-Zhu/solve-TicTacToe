...
class TicTacToeAgent():
    ...
    def getStrs(self, board):
        boards = [board, board[::-1], [board[col * 3 + row] for row in range(3) for col in range(3)], [board[col * 3 + row] for row in range(3) for col in range(3)][::-1]]
        for i in range(4):
            boards.append([boards[i][row * 3 + col] for col in range(3) for row in range(2, -1, -1)])
        for board in boards:
            if tuple(board) in self.pattern:
                return self.pattern[tuple(board)]

    def evaluationFunction(self, GameState, GameRules):
        newstr = ''
        for board in GameState.boards:
            if GameRules.deadTest(board) is not True:
                if self.getStrs(board): newstr = newstr + self.getStrs(board)
        if ''.join(sorted(newstr)) in ['cc', 'a', 'bb', 'bc']:
            return True

    def getAction(self, GameState, GameRules):
        for action in GameState.getLegalActions(GameRules):
            if self.evaluationFunction(GameState.generateSuccessor(action), GameRules):
                return action
